vocales = ["a","e","i","o","u","a"]

print (f"Lista: {vocales}")
print (f"La letra a esta en la posición: {vocales.index('a')}")
print (f"La letra a esta en la posición: {vocales.index('a')}")
print (f"La letra u en el rango 2-final esta en la posición: {vocales.index('u', 2)}")
print (f"La letra o en el rango 2-4 esta en la posición: {vocales.index('o', 2, 4)}")

print (f"La letra u en el rango 2-final esta en la posición: {vocales.index('z', 2)}")